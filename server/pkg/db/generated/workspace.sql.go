...
const listConnectedGitHubWorkspaces = `-- name: ListConnectedGitHubWorkspaces :many
SELECT id, name, slug, description, settings, created_at, updated_at, context, repos, issue_prefix, issue_counter, github_installation_id FROM workspace
WHERE github_installation_id IS NOT NULL
ORDER BY created_at ASC
`

func (q *Queries) ListConnectedGitHubWorkspaces(ctx context.Context) ([]Workspace, error) {
	rows, err := q.db.Query(ctx, listConnectedGitHubWorkspaces)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
